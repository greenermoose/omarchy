...
no_mise_path="$test_tmp/bin-without-mise:$ROOT/bin:$test_tmp/usr-bin-without-mise"
...
# The real installer is on PATH: the migration asks it whether a Hermes still
# answers before telling the user how to get one back.
run_migration() {
  : >"$mise_log"
  rm -f "$mise_log.removed" "$mise_log.unrequested"
  OMARCHY_TEST_MISE_LOG="$mise_log" \
    HOME="$test_home" \
    PATH="${OMARCHY_TEST_PATH:-$mock_bin:$ROOT/bin:$PATH}" \
...
# enough, and is left where it is.
...
[[ -f $saved_dir/hermes && $(cat "$hermes") == "$runtime_command" ]] || fail "the saved copy and the runtime's command are left alone"
